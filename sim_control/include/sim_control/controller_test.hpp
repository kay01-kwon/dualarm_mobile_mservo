...
class tracking_controller{
    ...
    void callback_state(const Odometry::ConstPtr& state_msg){
        est_pos_vel.x_est = state_msg -> pose.pose.position.x;
        est_pos_vel.y_est = state_msg -> pose.pose.position.y;

        est_pos_vel.qw_est = state_msg -> pose.pose.orientation.w;
        est_pos_vel.qx_est = state_msg -> pose.pose.orientation.x;
        est_pos_vel.qy_est = state_msg -> pose.pose.orientation.y;
        est_pos_vel.qz_est = state_msg -> pose.pose.orientation.z;
